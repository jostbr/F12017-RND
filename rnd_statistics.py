...
p_fails = [0.375, 0.4375, 0.5]           # Probability an upgrade will fail first attempt
...
tt_costs_1 = np.zeros((len(cost_upgrades), len(p_fails)))   # Total costs from all departments (cost upg)
tt_costs_2 = np.zeros((len(fail_upgrades), len(p_fails)))   # Total costs from all departments (fail upg)
...
for num_pf in range(len(p_fails)):
    for curr_dep_type in dp.Department.departments:     # For each department
        ...
        tt_costs_1[:, num_pf] += total_costs_1
        tt_costs_2[:, num_pf] += total_costs_2
        
        if (p_fails[num_pf] == p_fails[-1]):
            ...
            stats_str += format_str.format("Total cost:", max_dep.total_cost, "Cost upgs:",
                max_dep.cost_reduc, "Fail upgs:", max_dep.fail_reduc, "Initial fail pob:", p_fails[num_pf])

            stats_str += "\n\nParameters giving minimum cost:"
            stats_str += format_str.format("Total cost:", min_dep.total_cost, "Cost upgs:",
                min_dep.cost_reduc, "Fail upgs:", min_dep.fail_reduc, "Initial fail pob:", p_fails[num_pf])
...
ax_1.legend(loc = "upper right", fontsize = 10)
ax_15.grid(False)
...
ax_2.legend(bbox_to_anchor = (1, 0.15), loc = "lower right", fontsize = 10)
ax_25.grid(False)
ax_25.set_ylabel("Seasons until R&D completion", fontname = "serif", fontsize = 11)

total_colors = ["#333333", "#848484", "#C1C1C1"]    # Colors for total plots

for i in range(tt_costs_1.shape[1]):
    ax_3.plot(cost_upgrades, tt_costs_1[:, i], marker = "o",
        linewidth = 2, color = total_colors[i], label = "Fail prob: {}".format(p_fails[i]))

for i in range(tt_costs_2.shape[1]):
    ax_4.plot(fail_upgrades, tt_costs_2[:, i], marker = "o",
        linewidth = 2, color = total_colors[i], label = "Fail prob: {}".format(p_fails[i]))
...
ax_4.legend(loc = "upper left", fontsize = 10)
